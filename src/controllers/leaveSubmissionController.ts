...
const leaveSubmissionController = {
    ...
    getSubmissionLogin: async (req: Request, res: Response) => {
      try {
        ...
        const submisssionCount = await LeaveSubmission.findAndCountAll({ where: { user_id: user_id } });
    ...
    acceptSubmission: async (req: Request, res: Response) => {
      try {
        ...
        const submission = await LeaveSubmission.findOne({ where: { id: submissionId } });
        ...
        if (!submission) {
          ...
          const leaveAllowances = await LeaveAllowance.findOne({ where: { user_id: submission.user_id } });
          if (!leaveAllowances) {
